...
const Hero = () => {
  ...
  return (
    <section id="home" className="relative bg-gradient-to-b from-gray-50 to-white overflow-hidden">
      ...
      {/* Package Showcase Section - Now Central */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            From custom boxes to smart mylar bags, we have the perfect packaging solution for your brand
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          {packageShowcase.map((pkg, index) => (
            <div
              key={index}
              className={`relative bg-gradient-to-br ${pkg.bgColor} rounded-3xl p-8 shadow-xl border border-gray-100 hover:shadow-2xl transition-all duration-300 cursor-pointer ${
                index === activePackage ? 'ring-2 ring-purple-500 scale-105' : 'hover:scale-102'
              }`}
              onClick={() => setActivePackage(index)}
            >
              {/* Package Visual */}
              <div className={`h-48 mb-6 text-transparent bg-gradient-to-br ${pkg.color} bg-clip-text`}>
                {pkg.icon}
              </div>
              
              {/* Package Info */}
              <h3 className="text-2xl font-bold text-gray-900 mb-3 text-center">{pkg.type}</h3>
